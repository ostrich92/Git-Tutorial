package com.project.OstrichCompany.logistics.production.dao;
...
public class MpsDAOImpl implements MpsDAO {
	...
	public ArrayList<MpsTO> selectMpsList(String startDate, String endDate, String includeMrpApply) {
		...
		try {
			...
			/*
			 * SELECT * FROM MPS WHERE TO_DATE(MPS_PLAN_DATE, 'YYYY-MM-DD') BETWEEN
			 * TO_DATE(?, 'YYYY-MM-DD') AND TO_DATE(?, 'YYYY-MM-DD')
			 */
			query.append("SELECT * FROM MPS \r\n" + "WHERE TO_DATE(MPS_PLAN_DATE, 'YYYY-MM-DD') \r\n"
	...
	public void insertMps(MpsTO bean) {
		...
		try {
			 ...
			 * values ('PS2018070301','CO2018070301-01','수주',null,'DK-01','디지털카메라
			 * NO.1','2018-07-03',null,'EA','2018-07-13',200,null,null)
			 */
			query.append("Insert into MPS \r\n" + "(MPS_NO, CONTRACT_DETAIL_NO, MPS_PLAN_CLASSIFICATION, \r\n"
					+ "SALES_PLAN_NO, ITEM_CODE, ITEM_NAME, MPS_PLAN_DATE, SCHEDULED_END_DATE, \r\n"
					+ "UNIT_OF_MPS, DUE_DATE_OF_MPS, MPS_PLAN_AMOUNT, MRP_APPLY_STATUS, DESCRIPTION) \r\n"
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	public void updateMps(MpsTO bean) {
		...
		try {
			...
			StringBuffer query = new StringBuffer();
			/*
			 * UPDATE MPS SET MPS_PLAN_CLASSIFICATION = ? , CONTRACT_DETAIL_NO = ? ,
			 * SALES_PLAN_NO = ? , ITEM_CODE = ? , ITEM_NAME = ? , UNIT_OF_MPS = ? ,
			 * MPS_PLAN_DATE = ? , MPS_PLAN_AMOUNT = ? , DUE_DATE_OF_MPS = ? ,
			 * SCHEDULED_END_DATE = ? , MRP_APPLY_STATUS = ? , DESCRIPTION = ? WHERE MPS_NO
			 * = ?
			 */
			query.append("UPDATE MPS SET \r\n" + "MPS_PLAN_CLASSIFICATION = ? , CONTRACT_DETAIL_NO = ? ,\r\n"
	...
	public void changeMrpApplyStatus(String mpsNo, String mrpStatus) {
		...
		try {
			 ...
			 */
			query.append("UPDATE MPS SET MRP_APPLY_STATUS = ? WHERE MPS_NO = ?");
