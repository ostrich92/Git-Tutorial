	...
	public ArrayList<BomDeployTO> selectBomDeployList(String deployCondition, String itemCode) {
		...
		try {
			...
			*/
			...
			query.append("ORDER SIBLINGS BY NO ) T1,\r\n"
					+ "( SELECT ITEM_CODE, ITEM_NAME, ITEM_CLASSIFICATION, UNIT_OF_STOCK, LOSS_RATE, LEAD_TIME FROM ITEM ) T2\r\n"
					+ "WHERE TRIM(T1.ITEM_CODE) = T2.ITEM_CODE\r\n" + "ORDER BY BOM_NO");
...
FROM
( SELECT * FROM BOM 
    WHERE ( ITEM_CODE = ? AND PARENT_ITEM_CODE = 'NULL' ) 
        OR PARENT_ITEM_CODE = ? ) B, 
...
*/
			...
			query.append("SELECT B.PARENT_ITEM_CODE, B.ITEM_CODE, B.NO, I.ITEM_NAME, I.ITEM_CLASSIFICATION, \r\n"
					+ "    C.DETAIL_CODE_NAME AS ITEM_CLASSIFICATION_NAME, B.NET_AMOUNT, B.DESCRIPTION\r\n" + "FROM\r\n"
					+ "    ( SELECT * FROM BOM WHERE ( ITEM_CODE = ? AND PARENT_ITEM_CODE = 'NULL' ) OR PARENT_ITEM_CODE = ? ) B, \r\n"
					+ "    ITEM I, CODE_DETAIL C\r\n" + "WHERE B.ITEM_CODE = I.ITEM_CODE \r\n"
					+ "    AND I.ITEM_CLASSIFICATION = C.DETAIL_CODE\r\n" + "ORDER BY NO");
	...
	public ArrayList<BomInfoTO> selectAllItemWithBomRegisterAvailable() {
		...
		try {
			...
			*/
			...
			query.append("WITH ALL_REGISTERED_BOM AS\r\n" + "	( SELECT ITEM_CODE \r\n"
					+ "		FROM ( SELECT * FROM BOM WHERE PARENT_ITEM_CODE = 'NULL' )\r\n"
					+ "		GROUP BY ITEM_CODE ) ,\r\n" + "\r\n" + "ALL_ITEM_AS_PRODUCT AS\r\n"
					+ "	( SELECT ITEM_CLASSIFICATION, ITEM_CODE, ITEM_NAME \r\n"
					+ "		FROM ITEM WHERE ITEM_CLASSIFICATION IN ( 'IT-CI', 'IT-SI' ) )\r\n" + "\r\n"
	...
	public void insertBom(BomTO bean) {
		...
		try {
			 ...
			 * values (?,?,?,?,?,?)
			 */
			query.append("Insert into BOM ( ITEM_CODE, PARENT_ITEM_CODE, NO, NET_AMOUNT, DESCRIPTION ) \r\n"
					+ "values (?, ?, ?, ?, ?)");
	...
	public void updateBom(BomTO bean) {
		...
		try {
			...
			StringBuffer query = new StringBuffer();
			/*
			 * UPDATE BOM SET NO = ? , NET_AMOUNT = ? , DESCRIPTION = ? WHERE
			 * PARENT_ITEM_CODE = ? AND ITEM_CODE = ?
			 */
			query.append("UPDATE BOM SET \r\n" + "NO = ? , NET_AMOUNT = ? , DESCRIPTION = ?\r\n"
	...
	public void deleteBom(BomTO bean) {
		...
		try {
			...
			query.append("DELETE FROM BOM WHERE PARENT_ITEM_CODE = ? AND ITEM_CODE = ?");
