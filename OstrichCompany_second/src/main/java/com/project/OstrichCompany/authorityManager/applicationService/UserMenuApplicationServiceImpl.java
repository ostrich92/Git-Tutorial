...
import com.project.OstrichCompany.authorityManager.dao.UserMenuDAO;
...
public class UserMenuApplicationServiceImpl implements UserMenuApplicationService {
	...
	public String getUserMenuCode(String workplaceCode, String deptCode, String positionCode,
		...
		HashMap<String, UserMenuTO> map = userMenuDAO.selectUserMenuCodeList(workplaceCode, deptCode, positionCode);
		StringBuffer menuCode = new StringBuffer();
		TreeMap<Integer, UserMenuTO> treeMap = new TreeMap<>();
		...
		while (nextKey != null || key == maxKey) { // while 반복문 시작
			...
			UserMenuTO previouesBean = null;
			UserMenuTO nextBean = null;
			...
			if (treeMap.lowerKey(key) != null) {
				...
				previouesBean = treeMap.get(previousKey);
				previousMenuLevel = previouesBean.getMenuLevel();
			}

			if (nextKey != null) {
				nextBean = treeMap.get(nextKey);
				nextMenuLevel = nextBean.getMenuLevel();
			} else {
				nextMenuLevel = 0;
			}
			...
			if (menuLevel == 2 && previousMenuLevel == 1 && nextMenuLevel == 2) {
				menuCode.append(
						"\t\t<ul>\r\n" + "\t\t\t<li><a class='aTag' href='" + url + "'>" + menuName + "</a></li>\r\n");
			}
